...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_anno', dest='input_anno', help='The input dir of annotations', type=str)
    ...
    xml_folder = args.input_anno

    for img_path in tqdm(img_list):
        img = cv2.imread(img_path)
        if img is None or not img.any():
            continue
        xml_path = os.path.join(xml_folder, (os.path.basename(img_path)).replace(".jpg", ".xml"))
